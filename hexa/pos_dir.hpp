...
//
// Copyright (C) 2013-2014, nocte@hippie.nu
//---------------------------------------------------------------------------
#pragma once
...
public:
    ...
    pos_dir(const pos_t& p, direction_type d)
        : pos(p)
        , dir(d)
    {
    }
        ...
        : pos(p)
    {
        if (facing.x == p.x + 1)
            dir = dir_east;
        else if (facing.x == p.x - 1)
            dir = dir_west;
        else if (facing.y == p.y + 1)
            dir = dir_north;
        else if (facing.y == p.y - 1)
            dir = dir_south;
        else if (facing.z == p.z + 1)
            dir = dir_up;
        else if (facing.z == p.z - 1)
            dir = dir_down;
        else
            throw std::runtime_error("cannot determine direction");
    }
    ...
    pos_t target() const { return pos + dir_vector[dir]; }
...
public:
    ...
    bool operator==(const pos_t& compare) const { return pos == compare; }
...
inline ostream& operator<<(ostream& str, hexa::direction_type d)
{
