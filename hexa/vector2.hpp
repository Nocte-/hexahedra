...
public:
    ...
    vector2(vector2<conv_t> conv)
        : x(static_cast<type>(conv.x))
        , y(static_cast<type>(conv.y))
    {
    }
...
template <typename type>
inline ostream& operator<<(ostream& str, const hexa::vector2<type>& vtx)
{
    return str << '(' << vtx.x << ' ' << vtx.y << ')';
}

template <>
inline ostream& operator<<(ostream& str, const hexa::vector2<int8_t>& vtx)
{
    return str << '(' << (int)vtx.x << ' ' << (int)vtx.y << ')';
}

template <>
inline ostream& operator<<(ostream& str, const hexa::vector2<uint8_t>& vtx)
{
    return str << '(' << (int)vtx.x << ' ' << (int)vtx.y << ')';
}

template <typename t>
inline string to_string(const hexa::vector2<t>& vtx)
{
    std::stringstream s;
    s << vtx;
    return s.str();
