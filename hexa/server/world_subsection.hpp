...
protected:
    ...
    void add(const world_vector& pos, const chunk& cnk) { cnks_[pos] = &cnk; }
    ...
    const chunk& get_chunk(const world_vector& pos) const
    {
        ...
        if (found == cnks_.end())
            throw std::runtime_error(
                "world_subsection_read::get_chunk: chunk not found");
...
protected:
    ...
    void add(const world_vector& pos, chunk& cnk) { cnks_[pos] = &cnk; }
...
public:
    ...
    chunk& center_chunk() { return get_chunk(center()); }
    ...
    chunk& get_chunk(const world_vector& pos) const
    {
        ...
        if (found == cnks_.end())
            throw std::runtime_error(
                "world_subsection_write::get_chunk: chunk not found");
