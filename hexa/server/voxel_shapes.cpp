...
std::set<world_vector> shape::chunks() const
{
...
aabb<vec3f> sphere::bounding_box() const
{
    vec3f r(radius_);
    return {center_ - r, center_ + r};
}

bool sphere::intersects(const aabb<vec3f>& box) const
{
    float dmin(0.0f);
    for (int i = 0; i < 3; ++i) {
...
std::set<world_vector> sphere::chunks() const
{
    std::set<world_vector> result;
...
bool sphere::is_inside(const vec3f& p) const
{
    return squared_distance(center_, p) < sq_radius_;
}

ellipsoid::ellipsoid(const vec3f& center, const vec3f& radii,
...
truncated_cone::truncated_cone(const vec3f& pt1, float radius1,
...
std::set<world_vector> union_shape::chunks() const
{
    std::set<world_vector> result;
    for (auto& p : shapes_) {
        auto m(p->chunks());
...
aabb<vec3f> union_shape::bounding_box() const
{
    return bbox_;
}

bool union_shape::is_inside(const vec3f& p) const
{
...
aabb<vec3f> difference_shape::bounding_box() const
{
    return a_->bounding_box();
}

bool difference_shape::is_inside(const vec3f& p) const
{
    return hexa::is_inside(p, bounding_box()) && a_->is_inside(p)
           && !b_->is_inside(p);
}
...
aabb<vec3f> intersection_shape::bounding_box() const
{
    return bbox_;
}

bool intersection_shape::is_inside(const vec3f& p) const
{
    return hexa::is_inside(p, bounding_box()) && a_->is_inside(p)
           && b_->is_inside(p);
}
