...
public:
     ...
     */
    camera(vector pos = vector(0, 0, 0), yaw_pitch look_dir = yaw_pitch(0, 0),
           float roll = 0.0f, float fov = 1.22f, float aspect_ratio = 1.0f,
    ...
    const matrix4<float>& mvp_matrix() const { return mvp_matrix_; }

private:
    /** Recalculate the model-view matrix. */
    void recalc_mvm();
