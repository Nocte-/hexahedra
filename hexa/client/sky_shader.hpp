...
protected:
    inline triangle make_triangle(unsigned int a, unsigned int b,
                                  unsigned int c)
    {
        triangle result;
        result[0] = a;
...
public:
    ...
    color operator()(vector pos) const
    {
        ...
        result.x()
            = term_.x()
              * (1.f + A.x() * std::exp(B.x() * one_over_cos_zenith_angle))
              * (1.f + C.x() * std::exp(D.x() * dist_sun)
                 + E.x() * cos_dist_sun_sq);

        result.y()
            = term_.y()
              * (1.f + A.y() * std::exp(B.y() * one_over_cos_zenith_angle))
              * (1.f + C.y() * std::exp(D.y() * dist_sun)
                 + E.y() * cos_dist_sun_sq);
...
private:
    ...
    {
        float chi((4.f / 9.f - turbidity_ / 120.f)
                  * (3.1415927f - 2.f * thetas_));

        // Zenith luminance in Kcd/m2
        zenith_color_.Y() = (4.0453f * turbidity_ - 4.9710f) * std::tan(chi)
                            - 0.2155f * turbidity_ + 2.4192f;

        if (zenith_color_.Y() < 0.001f)
            zenith_color_.Y() = 0.001f;

        float t2(turbidity_ * turbidity_);
        // vector4<float> th (thetas_*thetas_*thetas_, thetas_*thetas_,
        // thetas_, 1.f);
        vector4<float> th(0, 0, 0, 1.f);

        static const vector4<float> xcoef1(0.00616f, -0.00375f, 0.00209f,
                                           0.0f),
        ...
        zenith_color_.x() = t2 * dot_prod(xcoef1, th)
                            + turbidity_ * dot_prod(xcoef2, th)
                            + dot_prod(xcoef3, th);
        zenith_color_.y() = t2 * dot_prod(ycoef1, th)
                            + turbidity_ * dot_prod(ycoef2, th)
                            + dot_prod(ycoef3, th);
    }
    ...
    void compute_term()
    {
        ...
        term_.x() = zenith_color_.x()
                    / ((1.f + A.x() * std::exp(B.x()))
                       * (1.f + C.x() * std::exp(D.x() * thetas_)
                          + E.x() * cos_thetas_sq));
        term_.y() = zenith_color_.y()
                    / ((1.f + A.y() * std::exp(B.y()))
                       * (1.f + C.y() * std::exp(D.y() * thetas_)
                          + E.y() * cos_thetas_sq));
        term_.Y() = zenith_color_.Y()
                    / ((1.f + A.Y() * std::exp(B.Y()))
                       * (1.f + C.Y() * std::exp(D.Y() * thetas_)
...
{
public:
    atmosphere(vector sd = vector(0, 1, 0), float re = 6360e3, float ra =
6420e3,
               float hr = 7994, float hm = 1200)
        : betaR (5.5e-6, 13.0e-6, 22.4e-6)
        , betaM (21e-6, 21e-6, 21e-6)
        , Hr(hr), Hm(hm), radiusEarth(re), radiusAtmosphere(ra),
sunDirection(sd)
...
vector atmosphere::computeIncidentLight(const Ray<T>& r) const
{
    ...
    float g = 0.76;
    float phaseM = 3 / (8 * M_PI) * ((1 - g * g) * (1 + mu * mu))/((2 + g * g)
* pow(1 + g * g - 2 * g * mu, 1.5));
    for (unsigned i = 0; i < numSamples; ++i)
    {
        vector samplePosition = r(tCurrent + T(0.5) * segmentLength);
        float height = samplePosition.magnitude() - radiusEarth; // compute
optical depth for light
        float hr = exp(-height / Hr) * segmentLength; T hm = exp(-height / Hm)
* segmentLength; opticalDepthR += hr; opticalDepthM += hm; // light optical
depth
        Ray<T> lightRay(samplePosition, sunDirection);
        intersect(lightRay, radiusAtmosphere, lightRay.tmin, lightRay.tmax);
        float segmentLengthLight = lightRay.tmax / numSamplesLight,
tCurrentLight = 0;
        float opticalDepthLightR = 0, opticalDepthLightM = 0;
        unsigned j = 0;
        for (j = 0; j < numSamplesLight; ++j)
        {
            vector samplePositionLight = lightRay(tCurrentLight + T(0.5) *
segmentLengthLight);
        ...
        {
            vector tau = betaR * (opticalDepthR + opticalDepthLightR) + betaM *
...
boost::numeric::ublas::bounded_array<float, 5>>
    distribution_terms;
...
color compute(float x_zenith, float y_zenith, float luminence, float theta,
float phi,
              float sun_theta, float sun_phi, float turbidity)
{
    boost::numeric::ublas::vector<float,
boost::numeric::ublas::bounded_array<float, 2>>
                turbidity_vec;
    ...
    float skycolor_y (1.0 - std::exp(-luminance * distribution(prod(ydc,
turbidity_vec)) / 25.));
