...
void scene::view_distance(size_t distance)
{
...
void scene::move_camera_to(chunk_coordinates pos)
{
    ...
    if (manhattan_distance(old_pos, pos) < 5) {
        ...
        while (step != pos) {
            ...
            if (step.x < pos.x) {
                ++step.x;
                direction = 0;
            } else if (step.x > pos.x) {
                --step.x;
                direction = 1;
            } else if (step.y < pos.y) {
                ++step.y;
                direction = 2;
            } else if (step.y > pos.y) {
                --step.y;
                direction = 3;
            } else if (step.z < pos.z) {
                ++step.z;
                direction = 4;
            } else if (step.z > pos.z) {
                --step.z;
...
void scene::set(chunk_coordinates pos, const surface_data& surface,
...
void scene::place_finished_mesh(const finished_mesh& mesh)
{
    // Generate the VBOs, and throw out the old occlusion query object.
    if (!terrain_.set(mesh.pos, {mesh.opaque->make_buffer(),
...
void scene::request_chunk_from_server(chunk_coordinates pos) const
{
...
void scene::chunk_became_visible(chunk_coordinates pos)
{
    if (!terrain_.is_inside(pos))
        return;
    ...
    auto& m(game_.map());
    if (m.is_surface_available(pos) && m.is_lightmap_available(pos)) {
