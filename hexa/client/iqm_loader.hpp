...
load_meshes (const std::vector<char>& buf)
{
    ...
    for (size_t i {0}; i < hdr.num_vertexarrays; ++i)
    {
        ...
        case field_type::position:
            if (va.format != float_t || va.size != 3)
                throw std::runtime_error("iqm::read: positions must be 3 floats");
        ...
        case field_type::normal:
            if (va.format != float_t || va.size != 3)
                throw std::runtime_error("iqm::read: normals must be 3 floats");
        ...
        case field_type::texcoord:
            if (va.format != float_t || va.size != 2)
                throw std::runtime_error("iqm::read: texture coordinates must be 2 floats");
        ...
        case field_type::blendindices:
            if (va.format != ubyte_t || va.size != 4)
                throw std::runtime_error("iqm::read: blend indices must be 4 bytes");
        ...
        case field_type::blendweights:
            if (va.format != ubyte_t || va.size != 4)
                throw std::runtime_error("iqm::read: blend weights must be 4 bytes");
    ...
    else
    {
        ...
        for  (size_t i {0}; i < hdr.num_vertices; ++i)
        {
            result.vertex_buffer_col[i] = model::vertex_col(
                position + i * 3,
                colors ? colors + i * 3 : defaultcolor,
                normal ? normal + i * 3 : defaultnormal,
                blend_indices ? blend_indices + i * 4 : defaultblend,
                blend_weights ? blend_weights + i * 4 : defaultblend);
        }
    ...
    {
        auto& m (meshes[i]);
        result.meshes.emplace_back(m.first_triangle,
                                   m.num_triangles,
                                   &str[m.material]);
    }
    ...
    for(size_t i {0}; i < hdr.num_joints; ++i)
    {
        ...
        result.bones.emplace_back(
            vector3<float>(j.translate),
            quaternion<float>(j.rotate),
            vector3<float>(j.scale),
            j.parent,
