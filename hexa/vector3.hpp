...
public:
    ...
    {
        return self((x % mod + mod) % mod, (y % mod + mod) % mod,
                    (z % mod + mod) % mod);
    }
    ...
    {
        return self((x % mod.x + mod.x) % mod.x, (y % mod.y + mod.y) % mod.y,
                    (z % mod.z + mod.z) % mod.z);
    }
    ...
    const self operator-() const { return self(-x, -y, -z); }
};
...
{
    return vector3<t>(l.y * r.z - l.z * r.y, l.z * r.x - l.x * r.z,
                      l.x * r.y - l.y * r.x);
}
...
{
    return vector3<int>(round_to_zero(in.x), round_to_zero(in.y),
                        round_to_zero(in.z));
}
...
template <typename t>
inline ostream& operator<<(ostream& str, const hexa::vector3<t>& vtx)
{
    return str << '(' << vtx.x << ' ' << vtx.y << ' ' << vtx.z << ')';
}

template <>
inline ostream& operator<<(ostream& str, const hexa::vector3<int8_t>& vtx)
{
...
template <>
inline ostream& operator<<(ostream& str, const hexa::vector3<uint8_t>& vtx)
{
    return str << '(' << (int)vtx.x << ' ' << (int)vtx.y << ' ' << (int)vtx.z
               << ')';
}
...
template <typename t>
inline istream& operator>>(istream& str, hexa::vector3<t>& vtx)
{
    return str >> vtx.x >> vtx.y >> vtx.z;
}

template <typename t>
inline string to_string(const hexa::vector3<t>& vtx)
{
    std::stringstream s;
    s << vtx;
    return s.str();
