...
{
    return ((uint64_t)(ntohl((uint32_t)((x << 32) >> 32))) << 32)
...
public:
    ...
    {
        return (*this)(
            uint16_t(val.x + chunk_size * val.y + chunk_area * val.z));
    }
    ...
    self& operator()(const wfpos& val) { return (*this)(val.pos)(val.frac); }
    ...
    self& raw_data(const t& val, size_t elements)
    {
        ...
        if (!val.empty()) {
            ...
            const char* e_ptr(s_ptr
                              + sizeof(typename t::value_type) * val.size());
...
public:
    ...
    size_t bytes_read() const { return std::distance(first_, cursor_); }

    size_t bytes_left() const { return std::distance(cursor_, last_); }
    ...
    self& operator()(wfpos& val) { return (*this)(val.pos)(val.frac); }
...
deserializer<std::vector<char>> make_deserializer(iter_t first, iter_t last)
{
