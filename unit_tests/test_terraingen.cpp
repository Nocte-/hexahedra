...
struct fixture
{
    fixture()
        ...
        , ctx(vars)
    {
        set_global_variable("seed", 5.0);
        set_global_variable("one", 1.0);
        set_global_variable("two", 2.0);
    }

    ~fixture() { fs::remove_all("test.db"); }

    void setup(const std::string& file)
    {
        pt::ptree config;
        pt::read_json(file, config);
        init_terrain_gen(w, config);
    }
...
BOOST_AUTO_TEST_CASE(chunk_test)
{
    ...
    BOOST_CHECK_EQUAL(proxy.get_block(world_center - world_vector(0, 0, 1)),
                      1);
    // Check lower limit
    BOOST_CHECK_EQUAL(proxy.get_block({0, 0, 0}), 1);
    // Check upper limit
    BOOST_CHECK_EQUAL(proxy.get_block({0xffffffff, 0xffffffff, 0xffffffff}),
                      0);
}
...
BOOST_AUTO_TEST_CASE(hndl_1_test)
{
    ...
    auto func = compile_hndl("@distance_test");
    map_coordinates spot{10, 10};
    auto area = hndl_area_int16(*func, map_chunk_center + spot);
...
BOOST_AUTO_TEST_CASE(hndl_2_test)
{
    ...
    auto func = compile_hndl("@distance_div");
    map_coordinates spot{10, 10};
    auto area = hndl_area_int16(*func, map_chunk_center + spot);
...
BOOST_AUTO_TEST_CASE(hndl_3_test)
{
    ...
    auto func = compile_hndl("@global_test");
    map_coordinates spot{10, 10};
    auto area = hndl_area_int16(*func, map_chunk_center + spot);
...
BOOST_AUTO_TEST_CASE(hmgen_1_test)
{
    ...
    auto func = compile_hndl("@heightmap");

    // Make sure the height map is what we expect ("x:div(2):sub(0.49)")
    auto area = hndl_area_int16(*func, map_chunk_center);
    BOOST_CHECK_EQUAL(area(0, 0), 0);
    BOOST_CHECK_EQUAL(area(8, 0), 4);
    BOOST_CHECK_EQUAL(area(8, 8), 4);
    BOOST_CHECK_EQUAL(area(15, 15), 7);

    auto area2 = hndl_area_int16(*func, map_chunk_center + vec2i{1, 0});
    BOOST_CHECK_EQUAL(area2(0, 0), 8);
    BOOST_CHECK_EQUAL(area2(15, 15), 15);

    auto area3 = hndl_area_int16(*func, map_chunk_center + vec2i{2, 0});
    BOOST_CHECK_EQUAL(area3(0, 0), 16);
    BOOST_CHECK_EQUAL(area3(15, 15), 23);

    // Is the coarse height map calculated correctly?
    auto proxy = w.acquire_read_access();
    BOOST_CHECK_EQUAL(proxy.get_coarse_height(map_chunk_center),
                      world_chunk_center.z + 1);
    BOOST_CHECK_EQUAL(proxy.get_coarse_height(map_chunk_center + vec2i{1, 0}),
                      world_chunk_center.z + 1);
    BOOST_CHECK_EQUAL(proxy.get_coarse_height(map_chunk_center + vec2i{2, 0}),
                      world_chunk_center.z + 2);
    BOOST_CHECK_EQUAL(proxy.get_coarse_height(map_chunk_center + vec2i{3, 0}),
                      world_chunk_center.z + 2);
    ...
    BOOST_CHECK_EQUAL(cnk(9, 8, 0), 1);
    BOOST_CHECK_EQUAL(cnk(9, 8, 3), 1);
    BOOST_CHECK_EQUAL(cnk(9, 8, 4), 0);
    BOOST_CHECK_EQUAL(cnk(9, 8, 15), 0);    
    
    BOOST_CHECK_EQUAL(cnk(8, 9, 0), 1);
    BOOST_CHECK_EQUAL(cnk(8, 9, 3), 1);
    BOOST_CHECK_EQUAL(cnk(8, 9, 4), 0);
    BOOST_CHECK_EQUAL(cnk(8, 9, 15), 0);    
}
